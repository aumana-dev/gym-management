...
export default function ClientDashboard() {
  ...
  const timeline = [
    'Completaste sesión de HIIT con Camila Torres',
    'Actualizaste tu objetivo semanal de pasos',
    'Reservaste una sesión de fuerza para mañana',
    'Alcanzaste tu récord de consistencia de 14 días',
  ];
  ...
  return (
    <div className="min-h-screen bg-[#0F0F0F] px-4 py-8 text-zinc-100 sm:px-6">
      <div className="mx-auto max-w-7xl">
        ...
        <p className="mt-2 text-zinc-400">Tu progreso se ve fuerte. Mantén el ritmo esta semana.</p>

        <div className="mt-7 grid gap-4 sm:grid-cols-3">
          {[
            ['Sesiones Completadas', estadisticas.completadas],
            ['Próximas Sesiones', estadisticas.proximas],
            ['Racha Activa (días)', estadisticas.racha],
          ].map(([title, value]) => (
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          <section className="rounded-2xl border border-zinc-800 bg-[#1A1A1A] p-5">
            <h2 className="text-xl font-bold text-white">Reservas próximas</h2>
            <div className="mt-4 space-y-3">
              {proximasReservas.map((reserva) => (
                <div key={reserva.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-zinc-800 bg-[#0F0F0F] p-4">
                  ...
                  <button
                    ...
                    className="rounded-lg border border-zinc-700 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-zinc-300 transition hover:border-[#22C55E] hover:text-[#22C55E]"
                  >
                    Cancelar
                  </button>
                </div>
          ...
          <section className="space-y-6">
            <article className="rounded-2xl border border-zinc-800 bg-[#1A1A1A] p-5">
              <h2 className="text-xl font-bold text-white">Entrenadores recomendados</h2>
              <div className="mt-4 space-y-3">
                {recomendados.map((coach) => (
                  <div key={coach.id} className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-[#0F0F0F] p-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">{coach.especialidad} · ${coach.precioPorSesion}</p>
                    </div>
                  </div>
                ))}
              </div>
